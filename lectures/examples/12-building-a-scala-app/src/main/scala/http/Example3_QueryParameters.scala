...
object Example3_QueryParameters extends App:
  ...
  val averageTemperatureService = HttpRoutes.of[IO] {
    case GET -> Root / "weather" / "temperature" :? CountryQueryParamMatcher(country) +& YearQueryParamMatcher(year) =>
      Ok(
        getAverageTemperatureForCountryAndYear(country, year).map(
          s"Average temperature for $country in $year was: " + _
        )
      )
    case _ =>
  ...
  println(
    requestEntityUnsafe[String](averageTemperatureService)(get(uri"/weather/temperature?country=Bulgaria&year=2020"))
  )
  println(
    requestEntityUnsafe[String](averageTemperatureService)(get(uri"/weather/temperature?country=Bulgaria&year=asdasd"))
  )
  ...
  val routes2 = HttpRoutes.of[IO] { case GET -> Root / "temperature" :? OptionalYearQueryParamMatcher(maybeYear) =>
    maybeYear match
      case None =>
  ...
  object ValidatingYearQueryParamMatcher
      extends ValidatingQueryParamDecoderMatcher[Year]("year")(validatingYearQueryParamDecoder)

  val routes = HttpRoutes.of[IO] { case GET -> Root / "temperature" :? ValidatingYearQueryParamMatcher(yearValidated) =>
    yearValidated.fold(
