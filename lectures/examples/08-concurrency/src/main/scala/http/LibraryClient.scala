...
object LibraryApi:
  ...
  def listBooks: Future[List[BookSummary]] = for
    ...
    books <- Future.sequence(bookIds.map(retrieveBookSummary))
  yield books
  ...
  def retrieveBook(bookId: String): Future[Book] = for
    ...
    authors <- Future.sequence(authorIds.map(retrieveAuthor))
...
object LibraryClient:
  ...
  def selectBook: Future[Unit] = for
    ...
    _ <-
      ...
      else console.putStringLine("Goodbye :)!")
  yield ()

  def displayBooks(books: List[BookSummary]): Future[Unit] =
    Future
      .sequence(
  ...
  def retrieveAndDisplayBook(bookId: String): Future[Unit] = (for
  ...
  def main(args: Array[String]): Unit = Await.result(selectBook, Duration.Inf)
